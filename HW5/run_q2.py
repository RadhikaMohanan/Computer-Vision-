...
for itr in range(max_iters):
    ...
    for x1,y1 in batches:
        # forward
        hypo1 = forward(x1, params, 'layer1', sigmoid)
        probs = forward(hypo1, params, 'output', softmax)

        # loss
        loss, acc = compute_loss_and_acc(y1, probs)
        total_loss += loss
        total_acc += acc
...
for k,v in params.items():
    ...
    if 'W' in k:
        ...
        for i in range(P):
            for j in range(Q):
                v[i,j] += eps
                hypo1 = forward(x, params, 'layer1')
                probs = forward(hypo1, params, 'output', softmax)
                losspE, acc = compute_loss_and_acc(y, probs)
                v[i,j] -= 2*eps
                hypo1 = forward(x, params, 'layer1')
                probs = forward(hypo1, params, 'output', softmax)
                losssE, acc = compute_loss_and_acc(y, probs)
                params['grad_' + k][i,j] = (losspE-losssE)/(2*eps)
                v[i,j] += eps
    if 'b' in k:
        P = v.shape[0]
        for i in range(P):
            ...
            probs = forward(hypo1, params, 'output', softmax)
            losssE, acc = compute_loss_and_acc(y, probs)
            params['grad_' + k][i] = (losspE-losssE)/(2*eps)
            v[i] += eps
